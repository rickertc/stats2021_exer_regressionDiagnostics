...
\#`{r eval=FALSE} # check each of the packages in the list and install them if they're not installed already for (i in packages){   if(! i %in% installed.packages()){     install.packages(i,dependencies = TRUE)   }   # show each package that is checked   print(i) }`\#
...
![](Regression-Diagnostics-Assignment_files/figure-markdown_github/unnamed-chunk-3-1.png)
...
\#`{r} car::outlierTest(model.3) qqPlot(model.3, main="QQ Plot") leveragePlots(model.3)`\#
...
#\`\`\`{r} \#
<https://stats.idre.ucla.edu/wp-content/uploads/2019/02/R_reg_part2.html#(4)>
plot(model.3, which = 4, cook.levels = cutoff) influencePlot(model.3,
main=“Influence Plot”, sub=“Circle size proportional to Cook’s
Distance”) infIndexPlot(model.3)#

# standardized residuals with lines at cutoff points

res.std \<- rstandard(model.3) plot(res.std, ylab = “Standardized
Residual”, ylim=c(-3.5, 3.5)) abline(h = c(-3, 0, 3), lty=2) index \<-
which(res.std \> 3 \| res.std \< -3) text(index-20, res.std\[index\],
labels = dataset$id\[index\]) print(index)
